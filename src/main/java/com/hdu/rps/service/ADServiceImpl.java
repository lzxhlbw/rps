...
public class ADServiceImpl implements ADSercive {
    ...
    public void recommendedNotcheckedPass(int rdpno) {
        recommendedPerson = recommendedPersonMapper.selectByPrimaryKey(rdpno);
//        int userNo=recommendMapper.selectUserNoByRecommendedPersonID(rdpno);
//        scoreRule=scoreRuleMapper.selectByPrimaryKey(1);
//        counts=countsMapper.selectByUserNo(userNo);
//        counts.setCountsquantity(counts.getCountsquantity()+scoreRule.getPass1());//这里暂时用上pass1的数据，实际上应当是合适的步骤值
//        countsMapper.updateByPrimaryKey(counts);
    ...
    @Override
    public void userManageAddSubmit(String name, String phone, String email, String password, String sex) {
        ...
        if(user == null) {
            ...
            //下段开始插入初始为零的积分
            counts=new Counts();
            counts.setUserno(userMapper.selectByUserEmail(email).getUserno());
            counts.setCountsquantity(0);
            SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            counts.setCountstime(sdf.format(new Date()));
